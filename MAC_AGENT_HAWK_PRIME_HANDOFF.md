...
If `hawk-prime` appears as `hawk-prime-1` or similar, the tailnet likely still has an old stale `hawk-prime` machine record. Ask the user to delete the stale old `hawk-prime` from the Tailscale admin console, then ask the Linux-side agent to run:
